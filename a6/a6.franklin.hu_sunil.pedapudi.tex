  ...
  \part % a
    ...
    \begin{align*}
       \log \hat{P}(x)
        &= \log \left( \frac{1}{N} \sum\limits_{i=1}^N
          \frac{1}{b} \exp \left( -\frac{d(x,x_i)}{b} \right) \right) \\
        &\ge \frac{1}{N} \sum\limits_{i=1}^{N} \left( -\frac{d(x,x_i)}{b}
          - \log{b} \right) \\
        &= -\frac{1}{N} \sum\limits_{i=1}^N \frac{d(x,x_i)}{b} + \log{b}
    \end{align*}
    We now have a simple lower bound expression for the log-likelihood.
